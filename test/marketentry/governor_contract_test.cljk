  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kita Systems MNE"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kita Systems MNE" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest foreign-branch-registration-missing-is-held-and-unoverridable
  (testing "missing verified CRPS foreign-branch registration -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:foreign-branch-registration-missing} (-> (store/ledger db) last :basis)))
