(ns marketentry.marketentryllm
  ...
  the MNE public-sector market-entry compliance actor.
  ...
  cited), never a committed record or a real CEJN portal submission.
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world CEJN portal submission."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "requires-foreign-branch-registration?=" (:requires-foreign-branch-registration? e)
                        " foreign-branch-registration-verified?=" (:foreign-branch-registration-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-foreign-branch-registration? e))
                              (:foreign-branch-registration-verified? e)))
                   0.9 0.3)}))
