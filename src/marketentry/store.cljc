(ns marketentry.store
  "SSoT for the MNE market-entry compliance actor, behind a `Store`
...
(defn demo-data
  ...
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems MNE" :portal "CEJN"
             :base-fee 400000 :monthly-rate 25000 :monitoring-months 12
             :claimed-fee 700000.0
             :requires-foreign-branch-registration? true :foreign-branch-registration-verified? true
             :drafted? false :submitted? false
             :jurisdiction "MNE" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "CEJN"
             :base-fee 400000 :monthly-rate 25000 :monitoring-months 12
             :claimed-fee 700000.0
             :requires-foreign-branch-registration? true :foreign-branch-registration-verified? true
             :drafted? false :submitted? false
             :jurisdiction "ATL" :status :intake}
    "eng-3" {:id "eng-3" :operator "Minami Trading Podgorica" :portal "CEJN"
             :base-fee 400000 :monthly-rate 25000 :monitoring-months 12
             :claimed-fee 999000.0
             :requires-foreign-branch-registration? true :foreign-branch-registration-verified? true
             :drafted? false :submitted? false
             :jurisdiction "MNE" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "CEJN"
             :base-fee 400000 :monthly-rate 25000 :monitoring-months 12
             :claimed-fee 700000.0
             :requires-foreign-branch-registration? true :foreign-branch-registration-verified? false
             :drafted? false :submitted? false
             :jurisdiction "MNE" :status :intake}}})
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-foreign-branch-registration? foreign-branch-registration-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-foreign-branch-registration?) (assoc :engagement/requires-foreign-branch-registration? requires-foreign-branch-registration?)
    (some? foreign-branch-registration-verified?) (assoc :engagement/foreign-branch-registration-verified? foreign-branch-registration-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-foreign-branch-registration? :engagement/foreign-branch-registration-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-foreign-branch-registration? (boolean (:engagement/requires-foreign-branch-registration? m))
     :foreign-branch-registration-verified? (boolean (:engagement/foreign-branch-registration-verified? m))
